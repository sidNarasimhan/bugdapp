 ...
 *
 * This is the transaction equivalent of raceApprove() / raceSign().
 */
export async function raceConfirmTransaction(
  ...
  const clickConfirmOnPage = async (popup: any): Promise<void> => {
    await popup.bringToFront();
    await popup.waitForLoadState('domcontentloaded').catch(() => {});
  ...
  if (popup) {
    ...
    // No popup yet — open notification.html manually (skip wallet.confirmTransaction() — wastes 30s on MV3)
    const extId = getExtensionId();
    if (extId) {
      const notifPage = await context.newPage();
      await notifPage.goto(`chrome-extension://${extId}/notification.html`);
