 ...
 */
export async function handleDAppNetworkSwitch(page, context, targetNetwork) {
    ...
    if (targetNetwork) {
        networkSwitchSelectors.push(page.getByRole('button', { name: new RegExp(targetNetwork, 'i') }), page.locator(`button:has-text("${targetNetwork}")`));
    }
    // Try each selector
    for (const selector of networkSwitchSelectors) {
        try {
            if (await selector.first().isVisible({ timeout: 2000 }).catch(() => false)) {
                ...
                const approved = await handleMetaMaskPopup(context, /approve|confirm|switch/i, { timeout: 10000 });
                if (approved) {
 ...
 */
export async function switchNetworkViaMetaMask(page, context, network) {
 ...
 */
export async function verifyNetwork(page, expectedChainId) {
    try {
